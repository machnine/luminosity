...
def read_data_from_fusion(patient_local_id, **kwargs):
    ...
    sql = f'''
        select  p.patientid as patient_id
                ...
                ,d.beadid as bead_id
                ,replace(replace(convert(nvarchar(100), specabbr), '-,',''), ',-', '') as ab_sero
                ,replace(replace(convert(nvarchar(100), specificity), '-,', ''), ',-', '') as ab_mol
                ,rawdata as raw_value
                ,normalvalue as baseline_value
        from patient as p
            ...
            join product_detail as pd on pd.BeadID = d.BeadID and t.CatalogID = pd.CatalogID
        where 
            {kittype}
